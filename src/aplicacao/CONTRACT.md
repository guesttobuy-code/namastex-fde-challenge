...
| I-15 | `encaminhar_planos_indisponiveis` (issue #95) NUNCA chama `PortalDeCotacao.cotar` — quando `/api/planos` falha mas a `/quote` está de pé, cotar de verdade escolheria "essencial" no lugar do lead (`_payload_da_quote`, `plano_id or "essencial"`); a função constrói `ENCAMINHAR`/`QUOTE_INDISPONIVEL` direto e reusa `_registrar_decisao_do_turno` (dono único também de `conduzir_conversa`) — e é idempotente: se a conversa já está `aguardando_corretor`/`em_atendimento_humano`/`encerrada`, não grava um segundo `decisao`/`handoff`/`status_alterado` | `tests/aplicacao/test_servico_conversa.py::test_encaminhar_planos_indisponiveis_*`, `tests/interfaces/test_rotas_planos_indisponivel.py` |
